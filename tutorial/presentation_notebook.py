...
b_ids = b_des[b_des['Sex'] == 1]['Animal_ID']
a_ids = a_des[a_des['Sex'] == 1]['Animal_ID']


# In[30]:


df_a = df_a[df_a['Animal_ID'].isin(a_ids)].drop('Animal_ID', axis=1).T
df_b = df_b[df_b['Animal_ID'].isin(b_ids)].drop('Animal_ID', axis=1).T
...
n=3
fig = a_depths.plot_deepest(n=n, title=f'{n} Deepest Male Rat Growth Curves, Group A', return_plot=True, yaxis_title='Weight (g)')
fig.update_layout(width=750, height=750)
...
fig = a_depths.plot_outlying(n=n, title=f'{n} Outlying Male Rat Growth Curves, Group A', return_plot=True, yaxis_title='Weight (g)')
fig.update_layout(width=750, height=750)
fig.update_layout(yaxis_range=[0, 500])
...
fig = b_depths.plot_deepest(n=n, title=f'{n} Deepest Male Rat Growth Curves, Group B', return_plot=True, yaxis_title='Weight (g)')
fig.update_layout(width=750, height=750)
fig.update_layout(yaxis_range=[0, 500])
fig.write_image('rat_b_deepest.pdf')
fig.show()

fig = b_depths.plot_outlying(n=n, title=f'{n} Outlying Male Rat Growth Curves, Group B', return_plot=True, yaxis_title='Weight (g)')
fig.update_layout(width=750, height=750)
fig.update_layout(yaxis_range=[0, 500])
...
a_des = a_des[a_des['Sex'] == 1]
b_des = b_des[b_des['Sex'] == 1]
...
for i, g in enumerate(groups):
    fig = FunctionalDepth([g], K=2, quiet=False, relax=True).plot_deepest(n, return_plot=True, title=f'{n} Deepest Curves, Experimental Group {i+1}, Male')
    fig.update_layout(width=750, height=750)
...
fig = go.Figure(
    ...
    layout=go.Layout(title='P2 Homogeneity Coefficient for each Experimental Group, Male Rats')
)
...
# ## Band Visualization
# 
# Here, we visualize bands for two functions and two irregular functions, to demonstrate the geometric nature of band depth
...
y1 = np.sin(2 * np.pi * x)
y2 = np.sin(2.5*np.pi*x)

y3 = np.sin(2*np.pi*x)
y4 = np.sin(2*np.pi*x) + 0.5
y5 = np.sin(2*np.pi*x) + 0.2

fig, (ax1, ax2) = plt.subplots(1, 2, sharex=True, figsize=(20, 6))

ax1.plot(x, y3, color='black')
ax1.plot(x, y4, color='black')
ax1.fill_between(x, y4, y3)
...
ax1.plot(x, y3, color='black')
ax1.plot(x, y4, color='black')
ax1.plot(x, y5, color='black')
ax1.fill_between(x, y4, y3)

ax2.plot(x, y3, color='black')
ax2.plot(x, y4, color='black')
ax2.plot(x, y5 + np.random.normal(0, 0.2, len(x)), color='black')
ax2.fill_between(x, y3, y4)


# In[52]:


fig.savefig('irregular_bands.pdf')
